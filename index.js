...
async function run() {
  try {
    ...
    const bookingsCollection = client.db("hotelRoomBooking").collection("bookings");
    const reviewsCollection = client.db("hotelRoomBooking").collection("reviews");
    ...
    app.put("/bookings/:id", async (req, res) => {
      ...
      try {
        ...
        if (result.modifiedCount === 0) {
          return res.status(404).json({ error: "Booking not found or no changes." });
        }
    ...
    app.delete("/bookings/:id", async (req, res) => {
      ...
      try {
        ...
        res.status(200).json({ message: "Booking canceled successfully, and room is now available!" });
      } catch (error) {
    ...
    app.get("/rooms/:id/details", async (req, res) => {
      ...
      try {
        ...
        console.error("Error fetching room details with reviews:", error.message);
        res.status(500).json({ error: "Internal server error." });
      }
    });
